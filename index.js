...
app.get("/schedule_test", (req, res) => {
  ...
  var headline_url =
    base_url + "/top-headlines" + country + page_size + lang + api_key;
  var apple =
    base_url +
    "/everything?q=apple" +
    page_size +
    ...
    .then(
      axios.spread((...result) => {
        ...
        var add_facebook = new Promise((resolve, reject) => {
          db.collection("news")
            .add({ index: "facebook_" + date, data: facebook_data })
            .then(() => resolve("Added Facebook"))
            .catch((err) => reject(err));
        });

        var add_nvidia = new Promise((resolve, reject) => {
          db.collection("news")
            .add({ index: "nvidia_" + date, data: nvidia_data })
            .then(() => resolve("Added Nvidia"))
            .catch((err) => reject(err));
        });

        var add_qualcomm = new Promise((resolve, reject) => {
          db.collection("news")
            .add({ index: "qualcomm_" + date, data: qualcomm_data })
            .then(() => resolve("Added Qualcomm"))
            .catch((err) => reject(err));
        });
...
app.get("/calculate_sentiments/:company", async (req, res) => {
  ...
  var date =
    today.getFullYear() + "-" + (today.getMonth() + 1) + "-" + today.getDate();
  db.collection("news")
    .where("index", "==", req.params.company + "_" + date)
    .get()
    .then((snapshot) => {
      var data = [];
      var data_array = [];
